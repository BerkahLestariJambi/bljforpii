...
function UpdateProfile() {
  ...
  const handleOk = (value) => {
    ...
    const forms = { ...userData, ...value }
    for (const key in forms) {
      if (forms[key]) {
        if (key !== "foto") {
          formData.append(key, forms[key])
        } else {
          formData.append(key, forms[key].file)
        }
      }
    }
      ...
      .then((res) => {
        ...
        dispatch(
          setUser({
            ...userData,
            ...res?.data?.data,
            foto: res?.data?.data?.profile_image_url,
          })
        )
      ...
      <div className="p-4 sticky top-0">
        <Link to="/profile">
          <div className="flex items-center font-bold text-lg">
            ...
            <p className="ml-3 text-gray-600">Ubah Profil</p>
          </div>
        </Link>
          ...
          <div className="mt-4">
            ...
            >
              <Form.Item
                ...
                name="foto"
                rules={[
                  { required: true, message: "Photo Profile Wajib Diupload" },
                ]}
                className="w-full"
              >
                <Upload.Dragger
